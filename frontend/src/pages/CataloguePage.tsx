...
const tCatalogue: Record<string, any> = {
  EN: {
    directory: "SalleHub Halls Directory",
  ...
  FR: {
    directory: "Annuaire des salles SalleHub",
  ...
  RW: {
    directory: "Urutonde rw'ibyumba bya SalleHub",
...
export default function CataloguePage({ lang = "EN", halls, onNavigate }: CataloguePageProps) {
  ...
  const seoData = {
    EN: {
      title: "Browse SalleHub Halls - ChurchTrack",
    ...
    FR: {
      title: "Parcourir les salles SalleHub - ChurchTrack",
    ...
    RW: {
      title: "Shakisha ibyumba bya SalleHub - ChurchTrack",
  ...
  const filteredHalls = useMemo(() => {
    const publicHalls = halls.filter((hall) => hall.id !== 'church-sanctuary');
    if (!searchTerm.trim()) return publicHalls;
