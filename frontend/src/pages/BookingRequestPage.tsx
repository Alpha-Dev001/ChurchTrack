...
interface BookingRequestPageProps {
  lang?: string;
  hall: Hall;
  selectedParams: { date: string; timeSlot: string; duration?: string; guests: number; serviceType?: 'SalleHub' | 'ChurchTrack' };
...
export default function BookingRequestPage({ lang = "EN", hall, selectedParams, onNavigate, onSubmitSuccess }: BookingRequestPageProps) {
  ...
  const seoData = {
    EN: {
      title: `Book ${hall.name} - ChurchTrack | SalleHub Halls`,
    ...
    FR: {
      title: `Réserver ${hall.name} - ChurchTrack | SalleHub Halls`,
    ...
    RW: {
      title: `Gukodesha ${hall.name} - ChurchTrack | SalleHub Halls`,
  ...
  const {
    ...
    defaultValues: {
      ...
      phone: "",
      email: "",
      eventType: selectedParams.serviceType === "ChurchTrack" ? "Wedding" : "Wedding",
  ...
  const serviceType = selectedParams.serviceType || "SalleHub";
    ...
    try {
      const responseData = await safeFetchJson("/api/bookings", {
        ...
        body: JSON.stringify({
          ...
          serviceType: selectedParams.serviceType || "SalleHub",
      ...
      <div className="font-sans text-navy-800 text-left" id="booking-request-page-root">
        ...
        <div className="max-w-5xl mx-auto py-10 px-4">
          {serviceType === "ChurchTrack" && (
            <div className="mb-6 rounded-3xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-950 shadow-sm">
              <p className="font-semibold">ChurchTrack wedding booking selected.</p>
              <p className="mt-1 text-[13px] text-amber-900">Your wedding request will be submitted under the ChurchTrack service. The selected slot must follow ChurchTrack wedding availability rules.</p>
            </div>
          )}
          <div className="flex items-center gap-2 mb-6">
            <button
              onClick={() => onNavigate("visitor-hall-details", { hallId: hall.id, serviceType })}
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="space-y-6">
                ...
                <div className="space-y-3.5 text-xs text-navy-600 font-semibold border-b border-navy-100 pb-4 font-sans">
                  ...
                  {serviceType === "ChurchTrack" && (
                    <div className="flex justify-between">
                      ...
                      <span className="text-navy-900 font-extrabold">ChurchTrack Wedding</span>
                    </div>
                  )}
                  <div className="flex justify-between">
              ...
              <div className="bg-navy-50 rounded-2xl border border-navy-200/80 p-5 text-xs font-bold text-navy-500 space-y-2 flex gap-3 font-sans text-left">
                <HelpCircle className="w-5 h-5 text-navy-400 mt-0.5 shrink-0" />
                <div>
