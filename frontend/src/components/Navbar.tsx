    ...
    <header className="fixed top-0 left-0 right-0 z-50 w-full bg-white border-b border-navy-200 transition-all shrink-0" id="public-header-navigation">
      <div className="max-w-6xl mx-auto px-4 py-3.5 flex items-center justify-between gap-3">
        ...
        <Link
          ...
          id="brand-logo-btn"
          aria-label="SalleHub Home"
        >
          <div className="p-2 bg-navy-950 text-white rounded-lg">
            <SalleHubLogo size={20} className="text-white" />
          </div>
          <div className="text-left">
            <h1 className="text-lg font-black tracking-[0.18em] text-navy-950 font-serif leading-none">SalleHub</h1>
            <span className="text-[9px] text-navy-500 font-semibold uppercase tracking-[0.24em] leading-none block mt-0.5">Parish Venues</span>
          </div>
        </Link>
        ...
        <nav className="hidden md:flex items-center gap-1 text-[11px] font-semibold text-navy-600 tracking-[0.14em] uppercase font-sans" role="navigation" aria-label="Main navigation">
          {[
            { key: "visitor-home", label: tNavbar.home, id: "nav-link-home", path: "/" },
            { key: "visitor-catalogue", label: tNavbar.halls, id: "nav-link-halls", path: "/catalogue" },
            { key: "visitor-track", label: tNavbar.track, id: "nav-link-track", path: "/track" }
          ].map((item) => (
            <Link
              key={item.key}
              to={item.path}
        ...
        <div className="flex items-center gap-2">
          ...
          <Link
            to="/catalogue"
            className="hidden sm:block bg-navy-950 hover:bg-navy-900 text-white text-[11px] font-semibold px-4 py-2.5 rounded-lg transition duration-200 tracking-[0.14em] uppercase cursor-pointer font-sans"
            ...
            >
              <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 space-y-1 flex flex-col text-left">
                ...
                <Link
                  to="/catalogue"
                  onClick={() => onSetMobileMenuOpen(false)}
                  className={`px-3.5 py-3 rounded-lg text-xs font-semibold text-left transition cursor-pointer ${currentView === "visitor-catalogue" ? "bg-navy-950 text-white" : "text-navy-700 hover:bg-navy-50"
                    }`}
                >
                  {tNavbar.halls}
                </Link>
                <Link
                  to="/track"
                  onClick={() => onSetMobileMenuOpen(false)}
                  className={`px-3.5 py-3 rounded-lg text-xs font-semibold text-left transition cursor-pointer ${currentView === "visitor-track" ? "bg-navy-950 text-white" : "text-navy-700 hover:bg-navy-50"
                    }`}
                >
                  {tNavbar.track}
                </Link>
                ...
                <div className="border-t border-navy-200 pt-3 mt-1">
                  <Link
                    to="/catalogue"
                    onClick={() => onSetMobileMenuOpen(false)}
                    className="w-full text-center bg-navy-950 hover:bg-navy-900 text-white text-xs font-semibold py-3 rounded-lg uppercase tracking-[0.14em] cursor-pointer transition flex items-center justify-center gap-2"
                  >
                    <span>{tNavbar.bookHall}</span>
